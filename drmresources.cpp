...
static
std::tuple<int, int> extractPrimaryDisplaySetting(std::string prop) {
  auto commaPos = std::find(prop.begin(), prop.end(), ',');
  if (commaPos != prop.end()) {
    *commaPos = '\0';
    int type = atoi(prop.c_str());
    int index = atoi(prop.c_str() + std::distance(prop.begin(), commaPos) + 1);
    return std::make_tuple(type, index);
  }
  return std::make_tuple(DRM_MODE_CONNECTOR_Unknown, 0);
}

static
void bringDisplayToFront(std::vector<std::unique_ptr<DrmConnector>> &connectors, int type, int index) {
  for (int i=1;i<connectors.size();i++) {
    if (connectors[i]->type() == type && --index == 0) {
      if (connectors[i]->state() == DRM_MODE_CONNECTED) {
        std::swap(connectors[0], connectors[i]);
      }
      break;
    }
  }
...
int DrmResources::Init() {
  ...
  char primary_display[PROPERTY_VALUE_MAX];
  property_get("com.realbom.primary_display", primary_display, "");

  std::stable_sort(connectors_.begin(), connectors_.end(),
     [](const std::unique_ptr<DrmConnector>& l, const std::unique_ptr<DrmConnector>& r) -> bool {
    if (l->state() != r->state()) {
  ...
  if (primary_display[0] != '\0') {
    auto expectedPrimary = extractPrimaryDisplaySetting(primary_display);
    if (std::get<0>(expectedPrimary) > DRM_MODE_CONNECTOR_Unknown &&
        std::get<0>(expectedPrimary) <= DRM_MODE_CONNECTOR_DPI &&
        std::get<1>(expectedPrimary) > 0) {
      ALOGI("Trying to set primary display %s:%d",
        connector_type_names[std::get<0>(expectedPrimary)].name,
        std::get<1>(expectedPrimary));
      bringDisplayToFront(connectors_, std::get<0>(expectedPrimary), std::get<1>(expectedPrimary));
    }
  }
